...
<form action="actions/<?=$selectedClass?>.action.php" method="post">
    <fieldset>
        ...
        <label for="numero">Número</label>
        <br>
        <input required=true type="number" name="numero" id="numero" value="<?=$linha['numero']?>" min="0">
        <br>
        <?php
            }
        ?>
...
<table class="table">
    ...
    <tbody>
        <?php 
            $pdo = Conexao::getInstance();
            $consulta = $pdo->query("SELECT * FROM ".$selectedClass." WHERE numero LIKE '$consulta%'");
            while ($linha = $consulta->fetch(PDO::FETCH_ASSOC)) {
                $instance = new vaga($linha['id'],$linha['numero']);
        ?>
                <tr>
                    <th scope="row"><?php echo $instance->getId();?></th>
                    <td><?php echo $instance->getNumero();?></td>
                    <?php
                        if ($_SESSION['admin'] == 1) {
                    ?>
